...
const NS_ROLE = {
  primary: { label: "Primary holder", icon: "star" },
  full:    { label: "Member · full", icon: "user" },
  earn:    { label: "Member · earn only", icon: "user" },
  view:    { label: "View only", icon: "eye" },
};
const NS_HH_STATUS = { engaged: "success", building: "info", "at-risk": "danger" };
const NS_POOL_MODES = [
  { id: "pooled", label: "Pooled", sub: "All member activity contributes to one shared balance — any authorised member can redeem it" },
  ...
  return (
    <div>
      ...
      <div className="ns-card" style={{ marginBottom: 14 }}>
        <div className="ns-card__head"><div className="ns-card__title">Household members</div><span className="ns-muted" style={{ fontSize: 11 }}>Roles set by the primary holder</span></div>
        <div className="ns-table-wrap">
          <table className="ns-table">
            <thead><tr><th>Member</th><th>Role</th><th>Attendance · season</th><th className="ns-num">Points contributed</th><th>Status</th></tr></thead>
            <tbody>
              {hh.members.map((m, i) => {
      ...
      <div className="ns-card ns-card--pad">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {NS_POOL_MODES.map((p) => (
            <button key={p.id} className={"ns-memchip" + (mode === p.id ? " is-on" : "")} style={{ flex: 1, alignItems: "flex-start", padding: 12 }} onClick={() => setMode(p.id)}>
